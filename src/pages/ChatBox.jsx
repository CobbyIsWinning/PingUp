        ...
        <div className='space-y-4 max-w-4xl mx-auto'>
          {
            ...
            ((messages, index)=>(
              <div key={index} className={`flex flex-col ${messages.to_user_id !== 
                user._id ? 'items-start' : 'items-end'
              }`}>
                <div className={`p-2 text-sm max-w-sm bg-white text-slate-700 rounded-lg shadow
                  ${messages.to_user_id !== user._id ? 'rounded-bl-none' : 'rounded-br-none'}`}>
                  {
                  messages.message_type === 'image' && <img src={messages.media_url} 
      ...
      <div className='px-4'>
        <div className='flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto
          ...
          <input type="text" placeholder='Type a message...' 
           ...
           value={text} className='flex-1 bg-transparent outline-none placeholder:text-sm' 
          />
          <label htmlFor="image">
            {
