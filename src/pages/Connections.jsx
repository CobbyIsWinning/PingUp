  ...
  return (
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <p className='text-shadow-amber-600'>Manage your network and discover new 
          connections
        </p>
      </div>
           ...
           rounded-md p-1 bg-white shadow-sm'>
              {
                dataArray.map((tap)=>(
                  <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 text-sm
                    rounded-md transition-colors ${currentTab === tap.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
